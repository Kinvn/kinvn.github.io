        ...
        try {
            ...
            for (;;) { //开始循环
                ...
                if (p == head && tryAcquire(arg)) { 
                    //成功获取锁，将当前 Node 设置为 head，注意此时我们的 Node 替换了原来的 dummy head
                    setHead(node); 
...
```java
    private void unparkSuccessor(Node node) {
        ...
        Node s = node.next; //dummy head 的下个节点，第一个线程节点
        if (s == null || s.waitStatus > 0) {
            s = null;
            // s 如果为 null 或已经 CANCELLED，从队列尾部开始寻找第一个不为 CANCELLED 的线程
...
为什么这里要从尾部开始寻找`Node`?
从 [addWaiter](#aqsaddwaiter) `Mark A`处可以看到：
